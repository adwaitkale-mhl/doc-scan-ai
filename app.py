...
def main(): 
    # URL Title and Logo
    urllib.request.urlretrieve('https://www.metropolisindia.com/img/favicon.ico', "img.png")
    img = Image.open("img.png")
    st.set_page_config(page_title="Metropolis AI", page_icon=img)
    # Adding Background Image and Removing Watermark
    html_configurations()     
    
    # Title
    st.markdown("<h2>\nMetropolis AI</h2>", unsafe_allow_html = True)
    
    #Change Text Color
    #st.markdown("This text is <span style='color:#ff6600'>colored pink</span>", unsafe_allow_html=True)
    
    # Upload File Prompt
    file_list = st.file_uploader(":orange[**Please upload PDF or DOCX files here:**]", type=["pdf","docx"], accept_multiple_files=True) 
    ...
    if file_list:
      ...
      if user_question:
        ...
        for a in range(r):
          time.sleep(0.1)
          progress_bar.progress(a+1,text="**Analyzing**")
        ...
        for i in range(r,100):
          time.sleep(0.01)
          progress_bar.progress(i+1,text="**Analyzing**")
        progress_bar.progress(100, text="**Analysis Done**")
